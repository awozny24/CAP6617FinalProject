...
print('Device:', device)
startSize = src.shape[1]
endSize = startSize
model = models.ANN(batch_size, startSize, endSize).to(device)
...
# train model
model, train_loss, val_loss = tools.trainModel(model, optimizer, criteria, trainLoader, valLoader, Epochs=Epochs, path=path, device=device, patience=20)

# load best model
model = models.load_best_model(path, models.ANN(batch_size, startSize, endSize))
...
print(f"Test Loss: {testLossSum/len(testLoader)}")
